...
- After setting `remote.url`, run or rerun `cavemem install` for each client IDE. It writes remote MCP entries for Claude Code (`type: "http"`), Codex (`url` plus `http_headers: { Authorization: "Bearer <token>" }`), and OpenCode (`type: "remote"`) instead of stdio commands. OpenCode install then compares `opencode debug config --pure` in the CLI's current directory, which it identifies in its result. A match is confirmed; an unavailable command, malformed result, or a different effective entry produces a prominent secret-safe warning without undoing or failing the completed filesystem install. `install` only touches OpenCode's canonical global config; a difference may be an intentional project, managed, environment-provided, or legacy override, and Cavemem leaves that source untouched for you to inspect or change.
- Every IDE carries the same static bearer in its config's `Authorization` header — written owner-only (0o600) by `install`, and never printed to install output. No client-side environment variable is needed; setups from 0.4.0/0.4.1 that exported `CAVEMEM_REMOTE_TOKEN` for Codex can drop it after re-running `cavemem install`.
...
Remote mode assumes a trusted subnet and uses a static bearer token; it does not add TLS, OAuth, or per-client tokens. The worker requires auth on every route except `/healthz`. `/api/*` and `/mcp` accept only the bearer token (`Authorization: Bearer <token>` or `X-Cavemem-Token`) — a browser navigation can't set either header. The viewer HTML routes (`/`, `/sessions/:id`) instead accept a one-time `?token=` query parameter, which the server trades for an `HttpOnly`, `SameSite=Strict` session cookie and a redirect that strips the value from the URL; `cavemem viewer` performs this handshake automatically. That query value is never the real bearer token — it's a short-lived, single-use nonce minted by the bearer-protected `POST /api/viewer-session`, so the credential that actually matters never lands in a URL, browser history, or (via a spawned browser opener) another local user's `ps` output. The cookie authenticates HTML routes only — it is never accepted on `/api/*` or `/mcp`, so a viewer tab open in a browser can't be used to drive the write path.
