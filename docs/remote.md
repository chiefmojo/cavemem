...
- After setting `remote.url`, run or rerun `cavemem install` for each client IDE. It writes remote MCP entries for Claude Code (`type: "http"`), Codex (`url` plus `http_headers: { Authorization: "Bearer <token>" }`), and OpenCode (`type: "remote"`) instead of stdio commands. OpenCode install then compares `opencode debug config --pure` with the intended entry and fails safely if another configuration source shadows it. `install` only touches OpenCode's canonical global config; inspect the effective configuration and remove or update the shadowing project, managed, environment-provided, or legacy entry yourself.
