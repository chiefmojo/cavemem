...
- **Dependency injection for tests**: `ServerDeps.fetchImpl` overrides `fetch`, used exclusively by the `enrich` tool handler; other tools are pure `MemoryStore` calls.
...
- Registered as an MCP server by the IDE installers (`packages/installers`) so editor agents can call it over stdio; no network listener is opened.
