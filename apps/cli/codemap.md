...
- **Node-runtime resolution + loud capture degradation (`opencode-bridge.ts`)**: OpenCode is a compiled Bun binary — inside it `process.execPath` is the IDE, not node, and Windows cannot `spawn()` a `.js` file. The bridge resolves a real node (its own `process.execPath` if usable → the install-time `cavemem-bridge.json#nodeBin` sidecar → PATH scan) and routes hook spawns through it; spawn failures are classified (`node-unavailable` / `cli-not-found` / `transient`), and the fatal kinds disable capture with a visible remedy printed to stderr **and** re-pushed into the system prompt instead of silently dropping every observation.
- **Command-level observability**: `doctor`/`status` print a per-IDE `summaries:` coverage line after the `db:` line — yellow when an IDE records sessions but zero turn summaries, the fingerprint of a stale hand-written OpenCode bridge plugin shadowing the bundled one (WP #210); `config unset` deletes settings keys entirely, so optional keys like `remote.url` disappear instead of lingering as `null` (WP #219).
