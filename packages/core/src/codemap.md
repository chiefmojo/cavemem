...
- **`memory-store.ts`** — exports `MemoryStore`, `MemoryStoreOptions { dbPath, settings }`, and the `Embedder` interface (`{ readonly model: string; readonly dim: number; embed(text): Promise<Float32Array> }`; declared here rather than in `types.ts` because it is part of the facade's search contract).
  ...
  - Sessions: `startSession({ id, ide, cwd, metadata? })` persists a sessions row through `storage.createSession` with `started_at = Date.now()`. The optional `metadata?: Record<string, unknown> | null` — added for remote mode, where hooks stamp the originating host (`host: hostname()`) into session metadata — is JSON-stringified into the row's `metadata` column; absent or `null` stores `null`. `endSession(id)` delegates to storage. The domain `Session` type (`types.ts`) does not expose metadata; it lives only on the storage row.
  - `ensureSession(id)` is a private idempotency guard: it re-issues `storage.createSession` before every child insert because Claude Code does not guarantee `SessionStart` fires first (installing mid-session, hook-chain failure, resumed sessions); without it, inserts fail the foreign-key constraint. It always writes `ide: 'unknown'`, `cwd: null`, `metadata: null` — it materializes the row for the foreign key only and never carries caller metadata.
...
1. Hooks' SessionStart handler calls `MemoryStore.startSession({ id, ide, cwd, metadata? })` → `storage.createSession` persists the sessions row (`INSERT OR IGNORE`, `started_at = Date.now()`); truthy `metadata` — the hooks runner stamps `host: hostname()` so remote-mode sessions record their originating host — is JSON-stringified into `sessions.metadata`, absent/`null` → `null`.
2. Producers (hook handlers) call `MemoryStore.addObservation`/`addSummary` → redaction → `ensureSession` → `compress` → `storage.insertObservation/insertSummary` → numeric id.
3. MCP `timeline` tool → `MemoryStore.timeline` → compressed rows (disclosure layer 1).
4. MCP `get_observations` tool → `MemoryStore.getObservations(ids, { expand: true })` → expanded rows (disclosure layer 2); default expand value comes from `settings.compression.expandForModel`.
5. MCP/CLI search → `MemoryStore.search(query, limit?, embedder?)`: `cap = limit ?? settings.search.defaultLimit`; `searchFts(query, cap * 2)` gives keyword hits; when an embedder is supplied and `settings.embedding.provider !== 'none'`, `allEmbeddings({ model, dim })` loads persisted vectors, the query is embedded, per-id BM25/cosine scores are merged and blended by `hybridRank(..., settings.search.alpha)`; vector-only hits get snippet/session info fetched from storage. Any degrade condition (no embedder, no vectors, `qvec.length !== embedder.dim`) returns the keyword slice untouched.
