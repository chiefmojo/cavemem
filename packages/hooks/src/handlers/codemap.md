...
- **`sessionStart` is idempotent** — Claude Code re-fires SessionStart on resume/clear/compact with the same `session_id`, so `store.startSession` must tolerate duplicates. It persists `input.metadata` into the session row (`metadata: input.metadata ?? null`) — by dispatch time `../runner.ts` has already stamped the originating machine's hostname into `metadata.host`, so sessions record where they were produced; this is what makes multi-machine (remote-mode) history distinguishable. Context injection happens only when `input.source === 'startup'` (on resume/clear/compact the agent already has its own context; a "Prior-session context" preface would be noisy and possibly stale). The fetch scopes by `cwd` in SQL — `store.storage.listSessions(20, { cwd })` (WP #209): an earlier machine-wide fetch with JS-side filtering let 20 unrelated recent sessions evict the current project's history and routinely returned zero hints (issue #39). The fixed-N window is not fully gone (the 20-row fetch still caps reach; same class as #39, strictly rarer), and the scan walks at most `MAX_CANDIDATES_SCANNED` = 10 of the most-recent sessions to bound how stale injected context can get; summary-less candidates are skipped before the 3-hint cap so newer bare sessions can't crowd out a summarized one. Each hint attaches at most one summary via `store.storage.listSummaries`, joined under a `## Prior-session context` heading.
