...
- prior-context.ts — shared prior-session hint scan (sessionStart + worker /api/context); endedOnly skips still consume the scan cap
...
- Public surface is the single `package.json#exports` entry: `runHook`, `ensureWorkerRunning`, remote target/client/error APIs, spool APIs, the five handlers, `buildPriorContext` (the shared hint scan behind `sessionStart` and the worker's `GET /api/context` priming route), and their hook/spool types.
- Tests: `test/runner.test.ts` covers local, injected, performance, and remote branches; `test/remote.test.ts` covers target validation, auth, non-2xx, and timeout; `test/spool.test.ts` covers cap/order/partial drain/locking/malformed rows; `test/post-tool-use.test.ts` covers capture filtering and bounded extraction; `test/prior-context.test.ts` covers the shared hint scan (cwd scoping, scan-cap accounting, `endedOnly`, `preferSessionScope`); `test/stop.test.ts` covers the dropped-summary debug diagnostic and the turn-summary write.
