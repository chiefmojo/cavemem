...
- 845eee3: Verify OpenCode's effective Cavemem MCP configuration after install, and advertise memory tools only while the MCP server is connected.
- a41ec3d: Persist stable PATH links to the running Node interpreter in IDE integrations. Add read-only installer diagnostics to doctor for missing or Homebrew-version-pinned Node paths, with an IDE-specific reinstall command.
