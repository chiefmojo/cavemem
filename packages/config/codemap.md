...
- `SettingsSchema` is a strict zod object; every field carries a `.describe()` string and a `.default()`. Adding a new field there automatically makes it appear in `cavemem config show` and `settingsDocs()` — no parallel docs to maintain. Remote mode (shared memory server) is configured here too: optional `remote.url` (its presence switches the machine into remote mode) and `remote.token`, `remote.timeoutMs` (default 1500), plus the server-side `workerHost` (default `127.0.0.1`) and `workerAllowedHosts` (host:port allowlist; loopback always accepted).
...
- Home resolution order: `CAVEMEM_HOME` (absolute or `~` only) → existing `~/.cavemem` → `XDG_DATA_HOME/cavemem` → Linux XDG default `~/.local/share/cavemem` → `~/.cavemem` fallback. `resolveDataDir` additionally expands a leading `~`. `workerTokenPath(dataDir)` joins the shared `worker-token` file under the resolved data dir (the worker creates/reads it; the CLI `viewer` command uses it for the cookie handshake).
...
- `cavemem config set`/`unset` (`apps/cli/commands/config.ts`) mutate a deep clone of the loaded settings via dotted-path helpers and re-validate the whole object with `SettingsSchema.safeParse` before `saveSettings`; `unset` of an optional key (e.g. `remote.url`) removes it from the file entirely (reverting to local mode), an absent-but-known path is an idempotent no-op success, and an unknown path errors.
