...
- Read path: `getObservations(ids)` bulk IN-fetch; `listSessions(limit, { cwd })` scopes recency in SQL (WP #209) — `null`/`undefined` stays machine-wide, any other value including `''` exact-matches the `cwd` column; `timeline(sessionId, aroundId?, limit)` returns a window centred on a row via two bounded queries merged in JS (a single UNION+LIMIT would let the "after" half starve the window); `listSummaries(sessionId)` orders `ts DESC, id DESC` so same-millisecond turn-summary/session-rollup ties resolve newest-inserted-first (WP #222); `summaryCoverage()` LEFT JOINs turn-scope summaries per IDE for `doctor`/`status`; `searchFts(query, limit, cwd?)` optionally joins `sessions` to restrict hits to one project cwd.
